//! quality-seed-alpha: under-lock seed lift vs 69a4fd7 baseline / GWT
...
fn main() {
    ...
    let out_dir = PathBuf::from("/workspace/rust-crate-test/quality-seed-alpha");
    ...
    let mut log = String::from("quality-seed-alpha rust runs (under-lock→1.0)\n");
    for name in clips {
        let inp = format!("{att}/{name}");
        let short = id8(&inp);
        let out = out_dir.join(format!("{short}_cand.mp4"));
